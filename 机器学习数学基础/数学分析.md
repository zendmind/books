...
$$
定理：设$\lbrace x_n \rbrace$是有界数列。则

1. $\overline{\lim_{n\to\infty}}x_n=H$的充分必要条件是：对任意给定的$\epsilon\gt0$
...
定理(Cauchy判别法)：设$\sum_{n=1}^{\infty}x_n$是正项级数，$r=\overline{\lim_{n\to\infty}}\sqrt[n]{x_n}$ ，则
...
(1)当$ \overline{\lim_{n\to\infty}}\frac{x_{n+1}}{x_n}=\bar{r}\lt1 $时，级数$\sum_{n=1}^{\infty}x_n$收敛；

(2)当$\lim{\overline{}}$时，级数发散；

(3)当或时，判别法失效，即级数可能收敛，也可能发散。
...
> $$
> \int_{\Omega}f(x_1,x_2,\cdots,x_n)dx_1dx_2\cdots dx_n\quad \text{或}\quad \int\dddot{\Omega}\int f(x_1,x_2,\cdots,x_n)dx_1dx_2\cdots dx_n
> $$
